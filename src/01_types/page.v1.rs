...
impl Page {
    ...
    pub fn apply_delta(&mut self, delta: &Delta) -> Result<(), PageError> {
        ...
        // Delta.mask is Vec<bool> → length == page size
        // But we store dirty mask as bitmask: 1 byte per 8 data bytes
        if delta.mask.len() != self.data.len() {
            return Err(PageError::MaskSizeMismatch {
                expected: self.data.len(),
                found: delta.mask.len(),
            });
        }

        let mut payload_idx = 0usize;
        for i in 0..self.data.len() {
            let changed = delta.mask[i];
            if changed {
                if delta.is_sparse {
                    ...
                    self.data[i] = delta.payload[payload_idx];
                    payload_idx += 1;
                } else {
                    // Dense: payload[i] corresponds to data[i]
                    self.data[i] = delta.payload[i];
                }
...
impl Clone for Page {
    fn clone(&self) -> Self {
        Self {
            id: self.id,
            epoch: EpochCell::new(self.epoch().0),
...
pub enum PageError {
    ...
    // THIS IS THE ONLY ONE THAT WORKS FLAWLESSLY
