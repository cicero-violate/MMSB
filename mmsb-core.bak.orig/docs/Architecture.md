...
4. **Routing & propagation** — DeltaRouter enqueues masked writes, updates page epochs, records the delta into the TLog, and raises propagation events. `route_delta!(; propagate=false)` lets callers such as `batch_route_deltas!` defer propagation so multiple deltas share one downstream wave.
5. **Propagation** — PropagationEngine queries ShadowPageGraph to find dependent pages and composes transformations in topological order. Branchless command buffers define how each dependency reacts, and the PropagationQueue deduplicates recompute requests before executing them.
...
- **Lawful composition:** PropagationEngine now builds command buffers per wave, merges duplicated edge types per page, and executes them without re-locking the graph.
- **PropagationQueue:** A per-state queue tracks pending recomputations and deduplicates requests so the same page is only recomputed once per wave, eliminating redundant event emissions.
...
## Release & Future Work
- **Complete:** Core runtime, GPU runtime, graph engine, public API, monitoring, benchmark suite, documentation, and the first Phase 2 propagation optimization wave (command buffer aggregation + dedup queue).
- **In Progress:** Sparse delta SIMD acceleration and allocator fast paths (Phase 2 performance work).
- **Deferred:** Instrumentation expansions (SSA/IR serialization), unified memory QoL, and distributed runtimes.
