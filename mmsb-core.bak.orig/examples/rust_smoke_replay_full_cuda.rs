...
#[repr(C)] #[derive(Clone, Copy)] struct JudgmentTokenHandle { ptr: *const c_void }
#[repr(C)] #[derive(Clone, Copy)] struct AdmissionProofInput { delta_hash: *const i8, version: u32 }
#[repr(C)] #[derive(Clone, Copy)] struct ExecutionProofInput { delta_hash: *const i8, version: u32 }
...
unsafe fn append_with_proofs(tlog: TLogHandle, delta: DeltaHandle, intent: &str) {
    let delta_hash_value = delta_hash(&*(delta.ptr as *const mmsb_core::prelude::Delta));
    let token = issue_judgment(intent, &delta_hash_value);
    let delta_hash_c = CString::new(delta_hash_value).unwrap();
    let token_handle = JudgmentTokenHandle { ptr: &token as *const _ as *const c_void };
...
fn main() {
    ...
    unsafe {
        ...
        for &(id, size) in CUDA_PAGES {
            ...
            append_with_proofs(tlog, delta, "cuda_write");
        ...
        append_with_proofs(tlog, delta, "sparse_gpu");
