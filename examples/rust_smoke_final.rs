...
extern "C" {
    ...
    fn mmsb_page_metadata_size(handle: PageHandle) -> usize;
    fn mmsb_page_metadata_export(handle: PageHandle, dst: *mut u8, len: usize) -> usize;
    fn mmsb_page_metadata_import(handle: PageHandle, src: *const u8, len: usize) -> i32;
    ...
    fn mmsb_delta_free(handle: DeltaHandle);
    fn mmsb_delta_apply(page: PageHandle, delta: DeltaHandle) -> i32;
    fn mmsb_delta_id(handle: DeltaHandle) -> u64;
    fn mmsb_delta_page_id(handle: DeltaHandle) -> u64;
    fn mmsb_delta_epoch(handle: DeltaHandle) -> u32;
    fn mmsb_delta_is_sparse(handle: DeltaHandle) -> u8;
    fn mmsb_delta_timestamp(handle: DeltaHandle) -> u64;
    fn mmsb_delta_source_len(handle: DeltaHandle) -> usize;
    fn mmsb_delta_copy_source(handle: DeltaHandle, dst: *mut u8, len: usize) -> usize;
    fn mmsb_delta_mask_len(handle: DeltaHandle) -> usize;
    fn mmsb_delta_copy_mask(handle: DeltaHandle, dst: *mut u8, len: usize) -> usize;
    fn mmsb_delta_payload_len(handle: DeltaHandle) -> usize;
    fn mmsb_delta_copy_payload(handle: DeltaHandle, dst: *mut u8, len: usize) -> usize;

    fn mmsb_tlog_new(path: *const c_char) -> TLogHandle;
    fn mmsb_tlog_free(handle: TLogHandle);
    fn mmsb_tlog_append(handle: TLogHandle, delta: DeltaHandle) -> i32;
    fn mmsb_checkpoint_write(
        allocator: AllocatorHandle,
        log: TLogHandle,
        path: *const c_char,
    ) -> i32;
    fn mmsb_checkpoint_load(
        allocator: AllocatorHandle,
        log: TLogHandle,
        path: *const c_char,
    ) -> i32;
    fn mmsb_tlog_reader_new(path: *const c_char) -> TLogReaderHandle;
    fn mmsb_tlog_reader_free(handle: TLogReaderHandle);
    fn mmsb_tlog_reader_next(handle: TLogReaderHandle) -> DeltaHandle;
    ...
    fn mmsb_allocator_page_count(handle: AllocatorHandle) -> usize;
    fn mmsb_allocator_list_pages(
        handle: AllocatorHandle,
...
fn main() {
    // CLEAN UP ANY LEFTOVER GARBAGE FROM PREVIOUS RUNS
