...
mod tests {
    ...
    fn admission_proof() -> AdmissionProof {
        AdmissionProof {
    ...
    fn commit_without_valid_admission_proof_halts() {
        ...
        let admission = admission_proof();

        let err = commit_delta(&log, &admission, delta, None)
            .expect_err("expected commit failure");
        // Adjust assertion based on actual error kind
        assert_eq!(err.kind(), std::io::ErrorKind::Other);  // or specific kind
    }
