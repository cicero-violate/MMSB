...
write_u8(&mut file, edge_type as u8)?;
}
...
pub fn replay_structural_log(path: impl AsRef) -> io::Result<DependencyGraph> {
...
let edge_type = EdgeType::from(read_u8(&mut file)?);
dag.add_edge(from, to, edge_type);
}
1 => {
let from = PageID(read_u64(&mut file)?);
let to = PageID(read_u64(&mut file)?);
