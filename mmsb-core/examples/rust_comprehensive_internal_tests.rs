...
fn test_transaction_log() {
    ...
    let delta_hash_value = delta_hash(&delta);
    let token = issue_judgment("internal", &delta_hash_value);
    let proof = MmsbExecutionProof {
        version: EXECUTION_PROOF_VERSION,
        delta_hash: delta_hash_value,
        tool_call_id: "internal".to_string(),
        tool_name: "internal".to_string(),
        output: json!({}),
        epoch: 0,
    };
    tlog.append(&token, &proof, delta).unwrap();
