...
struct RustJudgmentTokenHandle
    ptr::Ptr{Cvoid}
end

struct RustAdmissionProofInput
    delta_hash::Cstring
    version::UInt32
end

struct RustExecutionProofInput
    delta_hash::Cstring
    version::UInt32
end
...
function rust_tlog_append!(
    handle::RustTLogHandle,
    token::RustJudgmentTokenHandle,
    admission::RustAdmissionProofInput,
    execution::RustExecutionProofInput,
    delta::RustDeltaHandle,
)
    ensure_rust_artifacts()
    ccall(
        (:mmsb_tlog_append, LIBMMSB),
        Cint,
        (RustTLogHandle, RustJudgmentTokenHandle, RustAdmissionProofInput, RustExecutionProofInput, RustDeltaHandle),
        handle, token, admission, execution, delta,
    )
